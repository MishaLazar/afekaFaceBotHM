...
$stmt = mysqli_prepare($con, "INSERT INTO comments (user_id, post_id, message) VALUES (?, ?, ?)");
if (!$stmt) {
    http_response_code(500);
    exit("SQL prepare statement resulted in an error: " . mysqli_error($con));
}

$rc = mysqli_stmt_bind_param($stmt, "iis", $_POST["user_id"], $_POST["post_id"], $_POST["message"]);
if (!$rc) {
    http_response_code(500);
...
$created_id = mysqli_insert_id($con);

if (!$created_id) {
    $response = array(
        'meta' => array(
            'error' => TRUE,
            'error_message' => 'commenting failed. SQL error: ' . mysqli_stmt_error($stmt)
        )
    );
} else {
    $comment  = array(
